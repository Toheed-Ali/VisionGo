/// Manage Security Devices Section — Embedded device manager.
///
/// A self-contained section widget (used inside [AccountScreen]) that
/// lists all active security pairings stored locally and verified
/// against Firebase.
///
/// For each paired device the user can:
///   - **Monitor devices**: view alerts by navigating to [SecurityMonitorScreen]
///   - **All devices**: remove (unpair) with a role-aware confirmation dialog
///     that either deletes the entire pairing (camera) or just the monitor node.
library;
...
/// Embeddable widget that displays paired camera/monitor devices
/// and provides actions to view alerts or remove pairings.
  ...
  Future<void> _deleteDevice(String code, String role) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          role == 'camera'
              ? 'Are you sure you want to remove this camera device?\\n\\nThis will delete the entire pairing and all alerts.\\n\\nNote: Monitors paired to this camera will stop receiving alerts.'
              : 'Are you sure you want to remove this monitor device?\\n\\nThis will stop you from receiving alerts, but the camera will continue monitoring.\\n\\nYou can pair again using the same code.',
    ...
    if (confirmed == true) {
      try {
        // CRITICAL FIX: Different behavior based on role
        if (role == 'camera') {
          // Camera deletion: Remove entire pairing from Firebase
          await _securityService.deletePairing(code);
        } else {
          // Monitor deletion: Only remove monitor's device data
          await _securityService.unpairMonitor(code);
        }
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                role == 'camera'
                    ? 'Camera device and pairing removed'
                    : 'Monitor device removed (camera still active)',
              ),
