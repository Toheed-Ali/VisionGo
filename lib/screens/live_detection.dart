/// Live Detection Screen — Real-time YOLO object detection via the camera.
///
/// Displays a full-screen camera preview and overlays bounding boxes
/// for every detected object. Detection runs on a 2.5-second periodic
/// timer by capturing a still image and feeding it through [YoloDetector].
///
/// Features:
///   - Flash toggle (torch mode)
///   - Detection count badge and scrollable label chips
///   - [LiveDetectionPainter] for coordinate-mapped bounding boxes
library;
...
/// Receives an already-initialised [CameraController] from the gallery screen
/// so the camera doesn’t need to be re-initialised.
class LiveDetectionScreen extends StatefulWidget {
  /// Pre-initialised camera controller passed from [MainGalleryScreen].
  final CameraController camera;

  const LiveDetectionScreen({super.key, required this.camera});
...
class _LiveDetectionScreenState extends State<LiveDetectionScreen> {
  ...
  Future<void> _initializeYolo() async {
    try {
      ...
      debugPrint('Error initializing Detection Model: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error initializing Detection Model: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            if (_detections.isNotEmpty && _previewSize != null) _buildBoundingBoxes(screenSize),
  ...
  Widget _buildBoundingBoxes(Size screenSize) {
    return Positioned.fill(
      child: CustomPaint(
        ...
        child: const Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Initializing Model...', style: TextStyle(color: Colors.white, fontSize: 16)),
            ],
          ),
        ),
      ),
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  _isInitialized ? 'LIVE Detection' : 'LOADING',
  ...
  Widget _buildNoDetections() {
    return Container(
      ...
      child: const Text('No objects detected', style: TextStyle(color: Colors.white70)),
    );
  }

  Widget _circleButton({required IconData icon, required VoidCallback onPressed, Color? color}) {
    return Container(
      decoration: BoxDecoration(
...
/// Custom [CustomPainter] that renders bounding boxes and confidence
/// labels on top of the camera preview, with correct aspect-ratio scaling.
class LiveDetectionPainter extends CustomPainter {
  ...
  LiveDetectionPainter(this.detections, this.previewSize, this.screenSize, this.cameraAspectRatio);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < detections.length; i++) {
      ...
      final labelRect = Rect.fromLTWH(rect.left, labelY, textPainter.width + 8, 20);
