/// Main Gallery Screen — Photo gallery with live-camera access.
///
/// Displays the device's photo library in a responsive grid and
/// provides a button to open the live camera detection view.
///
/// Key responsibilities:
///   - Requests camera, photo, and notification permissions on first load
///   - Loads all device images via [PhotoManager], sorted newest-first
///   - Auto-refreshes every 5 seconds to pick up newly captured photos
///   - Tapping a photo navigates to [ObjectDetectionScreen] for YOLO analysis
///   - The "Open camera" button launches [LiveDetectionScreen]
library;
...
/// Stateful gallery widget that manages permissions, camera initialisation,
/// and the photo grid lifecycle.
...
class _MainGalleryScreenState extends State<MainGalleryScreen> with WidgetsBindingObserver {
  ...
  /// Requests camera, photo-library, and notification permissions.
  ///
  /// On success, initialises the camera preview and loads gallery images.
  /// On denial, shows a dialog directing users to system settings.
  ...
  /// Initialises the first available camera for the live-detection button.
  ...
  /// Fetches images from the device gallery using [PhotoManager].
  ///
  /// Avoids unnecessary `setState` calls by comparing the new list
  /// with the existing one (checks count and first item ID).
  ...
  /// Navigates to [ObjectDetectionScreen] for the tapped image
  /// with a slide-and-fade page transition.
  ...
  /// Opens the [LiveDetectionScreen] if the camera is ready,
  /// otherwise shows a waiting snackbar.
