/// YOLO Object Detection Service for VisionGo.
///
/// Wraps a YOLOv8-nano TensorFlow Lite model (`yolov8n_float32.tflite`)
/// and provides static methods for running inference on both camera
/// frames and gallery image files.
///
/// Pipeline:
///   1. Load model + COCO class labels on first call to [initialize].
///   2. Convert the input image to a 640×640 normalised float tensor.
///   3. Run TFLite inference → output shape `[1, 84, 8400]`.
///   4. Post-process: confidence threshold (0.25) + NMS (IoU 0.45).
///   5. Return a list of [Detection] objects.
library;
...
/// Core YOLOv8 object detection engine backed by TensorFlow Lite.
///
/// All members are static so detection can be used anywhere without
/// instantiation. The interpreter and labels are lazily loaded once.
class YoloDetector {
  /// TFLite interpreter instance (lazy-loaded).
  static Interpreter? _interpreter;
  /// COCO class labels loaded from `assets/labels.txt`.
  static List<String>? _labels;
  /// Whether [initialize] has completed successfully.
  static bool _isInitialized = false;

  /// Model input resolution (width and height in pixels).
  static const int inputSize = 640;
  /// Minimum confidence score for a detection to be kept.
  static const double confidenceThreshold = 0.25;
  /// IoU threshold for Non-Maximum Suppression.
  static const double iouThreshold = 0.45;
  /// Number of object classes in the COCO dataset.
  static const int numClasses = 80;

  /// Loads the TFLite model and COCO labels from application assets.
  ///
  /// No-ops if already initialised. Throws on failure.
  ...
  /// Runs detection on a live [CameraImage] frame.
  ///
  /// Converts the camera format (YUV420 / BGRA / JPEG), preprocesses,
  ...
  /// Runs detection on an image file from the gallery or filesystem.
  ...
  /// Converts a raw [CameraImage] to the `image` library format.
  ///
  /// Dispatches to the correct converter based on the platform’s
  /// image format group (YUV420, BGRA8888, or JPEG).
  ...
  /// Converts YUV420 (common Android format) to an RGB [img.Image].
  ...
  /// Converts BGRA8888 (common iOS format) to an [img.Image].
  ...
  /// Resizes the image to [inputSize]×[inputSize] and normalises
  /// pixel values to the \[0, 1\] range, returning a 4-D float tensor.
  ...
  /// Executes the TFLite model on the preprocessed [input] tensor.
  ///
  /// Returns the raw output of shape `[84, 8400]` — 4 bbox values
  /// plus 80 class scores for each of the 8400 anchor boxes.
  ...
  /// Converts raw model output into [Detection] objects.
  ///
  /// Iterates over all 8400 anchors, filters by [confidenceThreshold],
  /// scales coordinates to the original image dimensions, and applies
  ...
  /// Removes overlapping detections of the same class using greedy NMS.
  ...
  /// Releases the TFLite interpreter and clears cached labels.
  static void dispose() {
...
/// A single detected object with its class label, confidence score,
...
/// Axis-aligned bounding box defined by top-left `(x1, y1)` and
/// bottom-right `(x2, y2)` corners in pixel coordinates.
class BoundingBox {
