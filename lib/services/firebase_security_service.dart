...
class FirebaseSecurityService {
  ...
  /// Unpair monitor device (removes only monitor's data, keeps camera pairing intact)
  Future<void> unpairMonitor(String code) async {
    try {
      debugPrint('FirebaseSecurityService: Unpairing monitor for code: $code');

      // Stop monitoring if this is the active pairing
      if (_currentMonitoringCode == code) {
        await stopMonitoring();
      }
      final user = _auth.currentUser;
      if (user == null) throw Exception('User not authenticated');
      // Remove ONLY this monitor's node (not all monitors)
      // Path: /security-pairings/{code}/devices/monitors/{userId}
      final monitorDeviceRef = _database
          .child('security-pairings')
          .child(code)
          .child('devices')
          .child('monitors')
          .child(user.uid); // Use userId to target specific monitor

      await monitorDeviceRef.remove();
      // Remove from local storage
      await _removePairingLocally(code);
      debugPrint('FirebaseSecurityService: Monitor unpaired successfully');
    } catch (e) {
      debugPrint('FirebaseSecurityService: Error unpairing monitor: $e');
      rethrow;
    }
  }
  ...
  Future<void> saveFCMToken(String code, String role, String fcmToken) async {
      ...
      final user = _auth.currentUser;
      if (user == null) throw Exception('User not authenticated');
      // Different paths for camera vs monitors
      DatabaseReference deviceRef;
      if (role == 'camera') {
        // Camera stays at /devices/camera
        deviceRef = _database.child('security-pairings').child(code).child('devices').child('camera');
      } else if (role == 'monitor') {
        // Monitors go to /devices/monitors/{userId}
        deviceRef = _database
            .child('security-pairings')
            .child(code)
            .child('devices')
            .child('monitors')
            .child(user.uid); // Use userId as key
      } else {
        throw Exception('Invalid role: $role');
      }
      await deviceRef.set({ // Use set instead of update to create if not exists
        'fcmToken': fcmToken,
  ...
  Future<List<Map<String, dynamic>>> getAllMonitorTokens(String code) async {
    try {
      debugPrint('FirebaseSecurityService: Getting all monitor tokens for code: $code');
      final monitorsRef = _database
          .child('security-pairings')
          .child(code)
          .child('devices')
          .child('monitors');

      final snapshot = await monitorsRef.get();
        ...
        if (data != null) {
          data.forEach((userId, monitorData) {
            if (monitorData is Map) {
              ...
              monitor['userId'] = userId; // Include userId in result
              monitors.add(monitor);
            }
          });
        }
      }
      debugPrint('FirebaseSecurityService: Found ${monitors.length} monitor(s)');
      return monitors;
    } catch (e) {
      debugPrint('FirebaseSecurityService: Error getting monitor tokens: $e');
      return [];
    }
  }
  ...
  try {
    ...
    if (role == 'camera') {
      deviceRef = _database
          ...
          .child('devices')
          .child('camera');
    } else if (role == 'monitor' && user != null) {
      deviceRef = _database
          .child('security-pairings')
          .child(code)
          .child('devices')
      ...
      return; // Invalid role or no user
    }
