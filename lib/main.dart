/// VisionGo - AI-Powered Object Detection & Security Camera Application
///
/// This is the main entry point for the VisionGo app. It initializes
/// Firebase services (Authentication, Realtime Database, Cloud Messaging),
/// sets up background notification handling, and determines the initial
/// screen based on onboarding and authentication state.
///
/// Architecture: Follows a layered separation of concerns:
///   - Presentation Layer: [screens/] — all UI widgets and state management
///   - Service Layer: [services/] — business logic (YOLO detection, Firebase,
///     notifications, FCM API)
///   - Data Layer: Firebase Realtime Database & SharedPreferences
library;
...
///
/// Must be a top-level function (not a class method) so that Flutter
/// can invoke it even when the app process has been terminated.
...
/// Root widget for the VisionGo application.
///
/// Manages the overall app lifecycle, including:
/// - Onboarding status check (first-time users see the walkthrough)
/// - Firebase Authentication state (signed-in users go to [HomeScreen])
/// - Restoration of any active security monitoring sessions
  ...
  Future<void> _checkMonitoringState() async {
    try {
      ...
      if (activeCode != null && kDebugMode) {
        debugPrint('Active monitoring: $activeCode');
      }
    } catch (e) {
      if (kDebugMode) debugPrint('Error checking monitoring state: $e');
    }
  }
